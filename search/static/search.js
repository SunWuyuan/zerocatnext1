...
function projecthtml( d) {
  ...
  <mdui-card clickable href="/scratch/play.html?id=${d.id}">
    <img src="${config.s3url}/scratch_slt/${d.id}"   onerror="this.onerror=null; this.src='/assets/images/scratchdefault.png';"
  ...
  <div href='/user.html?id=${d.authorid}' style="padding: 16px;">
    <img class="card-avatar" src="${config.s3url}/user/${userinfo.find(item => item.id === Number(d.authorid)).images||'1'}" />
      ...
      if (d.length) {
        ...
        for (var i = 0; i < d.length; i++) {
          $("#choose_user_box").append(`
              ...
              <mdui-list-item rounded onclick="choose_user_set(${d[i].id})" class='choose_item' id='choose_item_${d[i].id}'>${userlist[i].display_name}    <mdui-avatar slot="icon" src="${config.s3url}/user/${userlist[i].images}">
...
function choose_user_set(id) {
  ...
  $("#open_choose_user").html(`
    ...
    <mdui-list-item rounded  active>${useritem.display_name}    <mdui-avatar slot="icon" src="${config.s3url}/user/${useritem.images}">
